...
def dbanalyse_positions(db, accId):
    ...
    try:
        ...
        final_list=[]
        for i in range(len(lst)):
            sql = "SELECT ctId, ctExecId, ctConId, ctType, ctMultiplier, ctShares, ctPrice, ctDate, ctCommission, ctLiquidation, ctoptPrice, ctoptIV, ctoptDelta, ctoptGamma, " \
            ...
            for h in (y for y in range(j + 1, stop) if sign(execs[y][5]) != sign(execs[j][5])):
                new_k = h
                break
            k = min(new_k, stop)
            while j < stop:
                if abs(execs[j][5]) < abs(execs[k][5]):         # Comparació de les +/- shares
                    execs[j].append(execs[k])                   # append a la llista de j tota la llista de k com exec[j][19]
                    ...
                    aux[5] = execs[j][5] + execs[k][5]          # el número de shares que quedaran al nou element
                    execs[j][5] = - execs[k][5]                 # ajustem el número de shares a execs[j] - posició que tanca
                    execs[j].append(execs[k])                   # append a la llista de j tota la llista de k com exec[j][19]
                ...
                execs[j][18] = 'C'                                  # posarem el registre a tActive = (C)losed - SEMPRE
                new_j = stop
                for h in (x for x in range(j+1, stop) if execs[x][18] != 'D'):
                    new_j = h
                    for l in (y for y in range(new_j + 1, stop) if sign(execs[y][5]) != sign(execs[new_j][5])):
                        k = l
                        break
                    break
                j = min(new_j, stop)
...
if __name__ == '__main__':
    ...
    aux = get_executions(myib)
    print(aux)
    dbfill_executions(mydb, aux)                   # inserta les noves (de 1 a 7 dies) executions
