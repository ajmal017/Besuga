...
def fillfundamentalslist(stklst):
    ...
    try:
        ...
        for s in range(len(stklst)):
            ...
            try:
                fratios = dict(t.split('=') for t in fr.fundamentalRatios.split(';') if t)
            except:
                continue
            #for key, value in fratios.items():
            #    print("Ratio: ", key, "  ", str(value) + "\n")
            ...
            vyield = fratios.get("YIELD", "")  # Dividend yield
            vroepct = fratios.get("TTMROEPCT", "")  # return on equity %
            #print("vEPS1  ",veps1,"vBeta  ",vbeta,"vcurrency  ",vcurrency, "vFrac52Wk  ",vfrac52wk)
            ...
            if vepsnext != "":
                ...
                df= requestadditionalfundamentals(cnt)
                ...
                stklst2.append(df["TargetPrice"])
                stklst2.append(df["ConsRecom"])
                stklst2.append(df["ProjEPS"])
                stklst2.append(df["ProjEPSQ"])
                stklst2.append(df["ProjPE"])
...
def processpreselectedstocks(stklst):
    ...
    try:
        ...
        dfps2 = pd.DataFrame(listprestocks, columns=prestocks._fields)
        #print("dfps  ",dfps)
...
if __name__ == "__main__":
    ...
    ordrespassadeslist = []
    daysnewpositions = 95 # distancia en dies per noves posicions, comptant desde dia present
    print(sys.version)
    # creem instancia de IB
    myib = IB()
    #  creem instancia de connexió db
    #mydb = dbconnect()
    ...
    if acc == "besugapaper":
        ...
        user="mav"
        password = "BESUGA8888"
        mydb = dbconnect(localhost,dbs,user,password)
